...
export default function ChatPage() {
  ...
  return (
    <SidebarLayout>
      <main className="h-screen flex flex-col bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        {/* Header */}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center max-w-2xl mx-auto">
                ...
                <div className="relative mb-8">
                  <div className="w-24 h-24 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 rounded-3xl flex items-center justify-center shadow-2xl shadow-indigo-500/40 transform rotate-3 hover:rotate-0 transition-transform duration-300">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8 w-full">
                  <div className="flex flex-col items-center p-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
                    <ListTodo className="w-6 h-6 text-indigo-400 mb-2" />
                    <span className="text-xs text-slate-400">List Tasks</span>
                  </div>
                  <div className="flex flex-col items-center p-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
                    <Zap className="w-6 h-6 text-amber-400 mb-2" />
                    <span className="text-xs text-slate-400">Add Tasks</span>
                  </div>
                  <div className="flex flex-col items-center p-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {SUGGESTED_PROMPTS.map((prompt, idx) => (
                      <button
                        ...
                        className="flex items-center gap-3 p-4 rounded-xl bg-slate-800/60 border border-slate-700/50 hover:border-indigo-500/50 hover:bg-slate-800 transition-all duration-200 text-left group"
                      >
                        <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${prompt.color} flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform`}>
          ...
          {/* Input Area */}
          <div className="flex-shrink-0 border-t border-slate-700/50 bg-slate-900/90 backdrop-blur-sm p-4">
              ...
              <div className="flex gap-3">
                <div className="flex-1 relative">
                  <Input
                    ...
                    disabled={isLoading}
                    className="w-full py-4 px-5 pr-12 rounded-xl border-2 border-slate-700 focus:border-indigo-500 bg-slate-800/80 text-white placeholder:text-slate-500 shadow-lg transition-all duration-200"
                  />
                  <MessageSquare className="absolute right-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-slate-500" />
                </div>
                <Button
                  type="submit"
                  disabled={isLoading || !inputValue.trim()}
                  className="px-6 py-4 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 transition-all duration-200 text-white font-medium disabled:opacity-50 disabled:shadow-none"
                >
